...
class EdgeConfigPersistence(BasePersistence):
    ...
    with a single, correctly scoped Vercel API Token for all operations.
    Data is stored in a clean JSON format.
    """
    def __init__(self):
        super().__init__()
    ...
    async def get_bot_data(self) -> dict:
        return self._read_all_data().get("bot_data", {})
    async def update_bot_data(self, data: dict) -> None:
        self._write_items([{"operation": "update", "key": "bot_data", "value": data}])
