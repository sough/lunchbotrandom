...
def create_result_keyboard() -> InlineKeyboardMarkup:
    """Creates the full inline keyboard for search results."""
    keyboard = [
        [InlineKeyboardButton("Искать снова 🔁", callback_data="repeat_search")],
        [
            ...
            InlineKeyboardButton("Сменить город 🏙️", callback_data="change_city")
        ]
    ]
    return InlineKeyboardMarkup(keyboard)

async def perform_search_and_reply(update: Update, context: CallbackContext, is_new_search: bool = False):
    """Performs the search based on data in context and sends a formatted reply."""
    query = update.callback_query
    if query:
        await query.edit_message_text("_Ищу...")
...
async def start(update: Update, context: CallbackContext) -> None:
    ...
    if context.user_data.get('city') and context.user_data.get('last_address'):
        ...
        context.user_data['state'] = 'confirm_search'
        ...
        await update.message.reply_html("Привет! Я помогу найти место для обеда. Пожалуйста, напишите ваш город (например, <b>Алматы</b>).")
...
async def handle_text(update: Update, context: CallbackContext) -> None:
    ...
    elif state == 'confirm_search' and user_text.lower() in ['да', 'yes', 'ок']:
        context.user_data.pop('state', None)
        save_user_data(user_id, context.user_data)
        await perform_search_and_reply(update, context, is_new_search=True)
