...
async def get_coordinates(address: str) -> tuple | None:
    ...
    params = {"q": address, "key": os.getenv("DGIS_API_KEY"), "fields": "items.point"}
    try:
        ...
        if data.get("meta", {}).get("code") == 200 and data.get("result", {}).get("items"):
            ...
            logger.warning(f"2GIS API не нашел координат для адреса '{address}'.")
            return None
    except requests.RequestException as e:
        logger.error(f"Ошибка сети при запросе к 2GIS Geocode API: {e}")
        return None
...
async def get_random_lunch_place(lat: float, lon: float, radius_meters: int) -> dict | None:
    ...
    for page_num in range(1, 11):
        ...
        params = {'key': os.getenv("DGIS_API_KEY"), 'q': "кафе, ресторан, столовая", 'point': f'{lon},{lat}', 'radius': radius_meters, 'type': 'branch', 'fields': 'items.name,items.address_name,items.url', 'page_size': 10, 'page': page_num}
        try:
            response = requests.get(url, params=params)
            response.raise_for_status()
            data = response.json()
...
def create_result_keyboard() -> InlineKeyboardMarkup:
    keyboard = [[InlineKeyboardButton("Повторить поиск 🔁", callback_data="repeat_search"), InlineKeyboardButton("Сменить радиус 📏", callback_data="change_radius")]]
...
async def set_city(update: Update, context: CallbackContext) -> None:
    ...
    context.user_data.pop('last_coords', None)
...
async def handle_message(update: Update, context: CallbackContext) -> None:
    ...
    if 'city' not in context.user_data:
        ...
        if not coords:
            await update.message.reply_text("Не смог найти такой город. Попробуйте еще раз.")
            return
    ...
    if not coords:
        await update.message.reply_text("Не смог найти такой адрес.")
        return
...
async def radius_start(update: Update, context: CallbackContext) -> int:
    current_radius = context.user_data.get('radius_km', DEFAULT_RADIUS_KM)
    await update.message.reply_text(f"Текущий радиус поиска: {current_radius} км.\nОтправьте новое значение в километрах (например, 0.5 или 3).\n\nЧтобы отменить, введите /cancel.")
    return ASKING_RADIUS
...
async def radius_receive(update: Update, context: CallbackContext) -> int:
    ...
    try:
        ...
        await update.message.reply_text("Это не похоже на число. Пожалуйста, введите корректное значение (от 0.1 до 10).")
        return ASKING_RADIUS
