...
async def get_random_lunch_place(lat: float, lon: float, radius_meters: int) -> dict | None:
    ...
    if all_places:
        place_choice = random.choice(all_places); return {"name": place_choice.get("name", "Н/Д"), "address": place_choice.get("address_name", "Н/Д"), "url": place_choice.get("url", "")}
    return None
...
async def handle_message(update: Update, context: CallbackContext) -> None:
    ...
    if 'city' not in context.user_data:
        ...
        context.user_data['city'] = user_text; escaped_city = escape_markdown_v2(user_text)
